# WIBOT - Documentation Technique et Fonctionnelle

> **Version** : 1.0
> **Date** : 31 Décembre 2024
> **Public cible** : Équipe technique et semi-technique WIDIP

---

## Table des matières

1. [Présentation Générale](#1-présentation-générale)
2. [Structure du Projet](#2-structure-du-projet)
3. [Architecture Globale](#3-architecture-globale)
4. [Frontend - Interface Utilisateur](#4-frontend---interface-utilisateur)
5. [Backend - Orchestration et API](#5-backend---orchestration-et-api)
6. [Base de Données](#6-base-de-données)
7. [Prérequis et Dépendances](#7-prérequis-et-dépendances)
8. [Installation et Démarrage](#8-installation-et-démarrage)
9. [Expérience Utilisateur](#9-expérience-utilisateur)
10. [Pistes d'Amélioration](#10-pistes-damélioration)

---

## 1. Présentation Générale

### Qu'est-ce que WIBOT ?

**WIBOT** est l'assistant IA conversationnel de WIDIP, conçu pour aider les équipes techniques dans leurs tâches quotidiennes :

- 🔧 Support technique (GLPI, réseau, monitoring)
- 📚 Consultation de documentation technique
- 💻 Assistance au développement (Python, JS, SQL, Bash, n8n)
- 🎫 Résumé et analyse de tickets/incidents
- 🤖 Automatisation de tâches

### Vision simplifiée

```
┌─────────────────────────────────────────────────────────────────┐
│                         UTILISATEUR                              │
│                    (Navigateur Web)                              │
└─────────────────────────────┬───────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│                     WIBOT FRONTEND                               │
│              (Interface de Chat React)                           │
└─────────────────────────────┬───────────────────────────────────┘
                              │ API REST
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│                     WIBOT BACKEND                                │
│         ┌──────────┐    ┌──────────┐    ┌──────────┐            │
│         │   n8n    │◄──►│ PostgreSQL│◄──►│ Mistral  │            │
│         │(Workflows)│    │   (BDD)   │    │   (IA)   │            │
│         └──────────┘    └──────────┘    └──────────┘            │
└─────────────────────────────────────────────────────────────────┘
```

---

## 2. Structure du Projet

### Arborescence du dossier WIBOT

```
WIBOT/
│
├── 📁 wibot-frontend/          # Application React (Interface)
│   ├── 📁 src/
│   │   ├── 📁 components/      # Composants réutilisables
│   │   ├── 📁 pages/           # Pages (Login, Chat)
│   │   ├── 📁 services/        # Appels API
│   │   ├── 📁 hooks/           # Logique métier réutilisable
│   │   ├── 📁 store/           # État global (Zustand)
│   │   ├── 📁 types/           # Définitions TypeScript
│   │   └── 📄 App.tsx          # Point d'entrée React
│   ├── 📄 package.json         # Dépendances npm
│   ├── 📄 vite.config.ts       # Configuration build
│   └── 📄 tailwind.config.js   # Thème visuel
│
├── 📁 wibot-backend/           # Orchestration n8n + Base de données
│   ├── 📁 workflows/           # Workflows n8n (JSON)
│   │   ├── 📄 auth_login.json
│   │   ├── 📄 chat_main.json
│   │   ├── 📄 get_conversations.json
│   │   └── ...
│   ├── 📁 rag-documents/       # Base de connaissances RAG
│   │   ├── 📁 documentation/
│   │   ├── 📁 procedures/
│   │   ├── 📁 clients/
│   │   └── 📁 tickets/
│   ├── 📁 frontend/            # Build React (servi par Nginx)
│   ├── 📄 docker-compose.yml   # Orchestration containers
│   ├── 📄 nginx.conf           # Configuration reverse proxy
│   └── 📄 init.sql             # Initialisation BDD
│
├── 📁 Documentation/           # Documentation existante
│
├── 📄 start.sh / start.bat     # Scripts de démarrage
├── 📄 stop.sh / stop.bat       # Scripts d'arrêt
├── 📄 FICHIER PROJECT.md       # Spécifications frontend
└── 📄 PROJECT_BACKEND.md       # Spécifications backend
```

### Schéma visuel de la structure

```
                              WIBOT/
                                │
            ┌───────────────────┼───────────────────┐
            │                   │                   │
            ▼                   ▼                   ▼
     ┌─────────────┐    ┌─────────────┐    ┌─────────────┐
     │  FRONTEND   │    │   BACKEND   │    │    DOCS     │
     │   (React)   │    │    (n8n)    │    │ (Markdown)  │
     └─────────────┘    └─────────────┘    └─────────────┘
            │                   │
     ┌──────┴──────┐     ┌──────┴──────┐
     │             │     │             │
     ▼             ▼     ▼             ▼
 ┌───────┐    ┌───────┐ ┌───────┐  ┌───────┐
 │ src/  │    │public/│ │ work- │  │ rag-  │
 │       │    │       │ │ flows/│  │ docs/ │
 └───────┘    └───────┘ └───────┘  └───────┘
```

---

## 3. Architecture Globale

### Vue d'ensemble de l'architecture

```
┌─────────────────────────────────────────────────────────────────────────┐
│                           NAVIGATEUR WEB                                 │
│                         (Chrome, Firefox, etc.)                          │
└───────────────────────────────────┬─────────────────────────────────────┘
                                    │ HTTP/HTTPS
                                    ▼
┌─────────────────────────────────────────────────────────────────────────┐
│                        NGINX (Reverse Proxy)                             │
│                           Port 8080                                      │
│  ┌────────────────────────────┬─────────────────────────────────────┐   │
│  │                            │                                      │   │
│  │  Fichiers statiques        │   Requêtes API (/webhook/*)         │   │
│  │  (HTML, CSS, JS)           │                                      │   │
│  │         │                  │              │                       │   │
│  └─────────┼──────────────────┼──────────────┼───────────────────────┘   │
│            │                  │              │                           │
│            ▼                  │              ▼                           │
│    ┌───────────────┐          │      ┌───────────────┐                  │
│    │   Frontend    │          │      │     n8n       │                  │
│    │  (Build React)│          │      │   Port 5678   │                  │
│    └───────────────┘          │      └───────┬───────┘                  │
│                               │              │                           │
└───────────────────────────────┼──────────────┼───────────────────────────┘
                                │              │
                                │              ▼
                                │      ┌───────────────┐
                                │      │  PostgreSQL   │
                                │      │  Port 5433    │
                                │      │ (+ pgvector)  │
                                │      └───────┬───────┘
                                │              │
                                │              ▼
                                │      ┌───────────────┐
                                │      │  Mistral API  │
                                │      │  (Cloud IA)   │
                                │      └───────────────┘
                                │
                  RÉSEAU DOCKER (wibot-network)
```

### Flux de communication principal

```
┌──────────────────────────────────────────────────────────────────────────┐
│                        FLUX D'UN MESSAGE CHAT                            │
├──────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  1. UTILISATEUR                                                          │
│     │ Tape "Comment résoudre une panne internet ?"                       │
│     ▼                                                                    │
│  2. FRONTEND (React)                                                     │
│     │ - Capture le message                                               │
│     │ - Prépare la requête JSON                                          │
│     │ - Ajoute le token JWT                                              │
│     ▼                                                                    │
│  3. NGINX                                                                │
│     │ - Route vers /webhook/wibot/chat                                   │
│     ▼                                                                    │
│  4. N8N (Workflow chat_main)                                             │
│     │ - Vérifie le JWT                                                   │
│     │ - Vérifie le quota de tokens                                       │
│     │ - Charge l'historique (10 derniers messages)                       │
│     │ - Prépare le contexte pour l'IA                                    │
│     ▼                                                                    │
│  5. MISTRAL API                                                          │
│     │ - Reçoit le prompt + contexte                                      │
│     │ - Génère la réponse                                                │
│     │ - Retourne tokens consommés                                        │
│     ▼                                                                    │
│  6. N8N (suite)                                                          │
│     │ - Sauvegarde messages en BDD                                       │
│     │ - Met à jour le compteur de tokens                                 │
│     │ - Retourne la réponse                                              │
│     ▼                                                                    │
│  7. FRONTEND                                                             │
│     │ - Affiche la réponse                                               │
│     │ - Met à jour le compteur de tokens                                 │
│     │ - Scroll automatique                                               │
│     ▼                                                                    │
│  8. UTILISATEUR                                                          │
│     └ Lit la réponse formatée (Markdown, code coloré, etc.)              │
│                                                                          │
└──────────────────────────────────────────────────────────────────────────┘
```

---

## 4. Frontend - Interface Utilisateur

### Stack Technique Frontend

| Technologie | Version | Rôle |
|-------------|---------|------|
| **React** | 18.2.0 | Framework UI composants |
| **TypeScript** | 5.9.3 | Typage statique JavaScript |
| **Vite** | 7.2.4 | Bundler et serveur de dev |
| **Tailwind CSS** | 3.4.19 | Framework CSS utilitaire |
| **Zustand** | 5.0.9 | Gestion d'état global |
| **React Router** | 7.11 | Navigation SPA |
| **Axios** | 1.13.2 | Client HTTP |
| **React Markdown** | 10.1.0 | Rendu Markdown |
| **react-syntax-highlighter** | 16.1.0 | Coloration syntaxique |
| **react-dropzone** | 14.3.8 | Upload drag & drop |
| **Lucide React** | 0.562.0 | Icônes |
| **date-fns** | 4.1.0 | Manipulation de dates |

### Architecture des composants

```
┌─────────────────────────────────────────────────────────────────────────┐
│                              App.tsx                                     │
│                          (Routes + Auth)                                 │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│    ┌───────────────────────────────────────────────────────────────┐    │
│    │                        Chat.tsx (Page)                         │    │
│    │  ┌─────────────────────────────────────────────────────────┐  │    │
│    │  │                     Header.tsx                           │  │    │
│    │  │  [Logo]  [Compteur Tokens]  [Menu Utilisateur] [Logout] │  │    │
│    │  └─────────────────────────────────────────────────────────┘  │    │
│    │  ┌────────────┬────────────────────────────────────────────┐  │    │
│    │  │            │                                             │  │    │
│    │  │ Sidebar    │           ChatWindow.tsx                    │  │    │
│    │  │            │                                             │  │    │
│    │  │ [+ Nouv.]  │    ┌─────────────────────────────────┐     │  │    │
│    │  │            │    │         Message.tsx              │     │  │    │
│    │  │ Conv 1     │    │  [Avatar] Utilisateur: "..."     │     │  │    │
│    │  │ Conv 2     │    └─────────────────────────────────┘     │  │    │
│    │  │ Conv 3     │    ┌─────────────────────────────────┐     │  │    │
│    │  │ ...        │    │         Message.tsx              │     │  │    │
│    │  │            │    │  [Avatar] WIBOT: "..."           │     │  │    │
│    │  │            │    │  ├── MarkdownRenderer.tsx        │     │  │    │
│    │  │            │    │  └── CodeBlock.tsx               │     │  │    │
│    │  │            │    └─────────────────────────────────┘     │  │    │
│    │  │            │                                             │  │    │
│    │  └────────────┴────────────────────────────────────────────┘  │    │
│    │  ┌─────────────────────────────────────────────────────────┐  │    │
│    │  │                    InputBar.tsx                          │  │    │
│    │  │  [Mode ▼] [Zone de texte...              ] [Envoyer 📤] │  │    │
│    │  │  [FileDropzone] [FilePreview]                           │  │    │
│    │  └─────────────────────────────────────────────────────────┘  │    │
│    └───────────────────────────────────────────────────────────────┘    │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

### Gestion d'état (Zustand Stores)

```
┌─────────────────────────────────────────────────────────────────────────┐
│                         STORES ZUSTAND                                   │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  ┌─────────────────┐   ┌─────────────────┐   ┌─────────────────┐        │
│  │   AUTH STORE    │   │ CONVERSATIONS   │   │   CHAT STORE    │        │
│  │                 │   │     STORE       │   │                 │        │
│  ├─────────────────┤   ├─────────────────┤   ├─────────────────┤        │
│  │ • user          │   │ • conversations │   │ • messages[]    │        │
│  │ • token (JWT)   │   │ • currentConv   │   │ • isLoading     │        │
│  │ • isAuthenticated│  │                 │   │ • tokensRemaining│       │
│  │                 │   │                 │   │ • chatMode      │        │
│  ├─────────────────┤   ├─────────────────┤   ├─────────────────┤        │
│  │ Actions:        │   │ Actions:        │   │ Actions:        │        │
│  │ • login()       │   │ • create()      │   │ • sendMessage() │        │
│  │ • logout()      │   │ • rename()      │   │ • loadMessages()│        │
│  │ • initAuth()    │   │ • delete()      │   │ • setMode()     │        │
│  └─────────────────┘   └─────────────────┘   └─────────────────┘        │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

### Modes de Chat disponibles

| Mode | Modèle IA | Usage recommandé |
|------|-----------|------------------|
| **Code** | Devstral | Optimisé pour le code et le développement |
| **Flash** | Devstral Small | Réponses rapides et économiques |
| **Pro** | Mistral Large | Qualité maximale, raisonnement complexe |

### Design System

```
┌─────────────────────────────────────────────────────────────────────────┐
│                           THÈME SOMBRE                                   │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│   Couleurs principales:                                                  │
│   ┌──────────────┬──────────────┬──────────────┐                        │
│   │  #1A1A1A     │  #141414     │  #0D0D0D     │                        │
│   │  (Fond)      │  (Fond sec.) │  (Code)      │                        │
│   └──────────────┴──────────────┴──────────────┘                        │
│                                                                          │
│   ┌──────────────┬──────────────┬──────────────┐                        │
│   │  #2A2A2A     │  #232323     │  #333333     │                        │
│   │  (Msg User)  │  (Msg Bot)   │  (Bordures)  │                        │
│   └──────────────┴──────────────┴──────────────┘                        │
│                                                                          │
│   Texte et accent:                                                       │
│   ┌──────────────┬──────────────┬──────────────┐                        │
│   │  #E5E5E5     │  #A0A0A0     │  #5B9EFF     │                        │
│   │  (Primaire)  │  (Secondaire)│  (Accent)    │                        │
│   └──────────────┴──────────────┴──────────────┘                        │
│                                                                          │
│   Dimensions:                                                            │
│   • Sidebar: 280px                                                       │
│   • Header: 60px                                                         │
│   • InputBar: 80px                                                       │
│   • Police: Inter, 15px (texte) / 14px (code)                           │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

---

## 5. Backend - Orchestration et API

### Stack Technique Backend

| Technologie | Version | Rôle |
|-------------|---------|------|
| **n8n** | Latest | Orchestration de workflows |
| **PostgreSQL** | 14 | Base de données relationnelle |
| **pgvector** | Extension | Recherche vectorielle (RAG) |
| **Nginx** | Alpine | Reverse proxy + serveur statique |
| **Docker** | Latest | Conteneurisation |
| **Docker Compose** | v2 | Orchestration multi-conteneurs |
| **Mistral API** | Cloud | Modèles IA (Large, Devstral) |

### Architecture des conteneurs Docker

```
┌─────────────────────────────────────────────────────────────────────────┐
│                       DOCKER COMPOSE                                     │
│                     (wibot-network)                                      │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │                    NGINX (wibot-nginx)                           │    │
│  │                      Port: 8080                                  │    │
│  │  • Sert les fichiers statiques React                            │    │
│  │  • Reverse proxy vers n8n (/webhook/*)                          │    │
│  │  • Rate limiting (10 req/s)                                     │    │
│  │  • Headers de sécurité                                          │    │
│  └─────────────────────────────────┬───────────────────────────────┘    │
│                                    │                                     │
│                                    ▼                                     │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │                      N8N (wibot-n8n)                             │    │
│  │                      Port: 5679                                  │    │
│  │  • Exécution des workflows                                      │    │
│  │  • Webhooks API REST                                            │    │
│  │  • Agent IA (Mistral)                                           │    │
│  │  • Gestion du contexte RAG                                      │    │
│  └─────────────────────────────────┬───────────────────────────────┘    │
│                                    │                                     │
│                                    ▼                                     │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │                 POSTGRESQL (wibot-postgres)                      │    │
│  │                      Port: 5433                                  │    │
│  │  • Stockage utilisateurs                                        │    │
│  │  • Stockage conversations et messages                           │    │
│  │  • Suivi consommation tokens                                    │    │
│  │  • Extension pgvector (embeddings)                              │    │
│  └─────────────────────────────────────────────────────────────────┘    │
│                                                                          │
│  Volumes persistants:                                                    │
│  • postgres_data → /var/lib/postgresql/data                             │
│  • n8n_data → /home/node/.n8n                                           │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

### Workflows n8n

```
┌─────────────────────────────────────────────────────────────────────────┐
│                        WORKFLOWS N8N                                     │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │                    AUTHENTIFICATION                              │    │
│  │  auth_login.json                                                 │    │
│  │  POST /webhook/auth/login                                        │    │
│  │  → Vérifie credentials → Génère JWT (8h)                        │    │
│  └─────────────────────────────────────────────────────────────────┘    │
│                                                                          │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │                    CHAT PRINCIPAL ⭐                              │    │
│  │  chat_main.json                                                  │    │
│  │  POST /webhook/wibot/chat                                        │    │
│  │  → JWT → Quota → Contexte → Mistral → Save → Réponse           │    │
│  └─────────────────────────────────────────────────────────────────┘    │
│                                                                          │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │                    GESTION CONVERSATIONS                         │    │
│  │  • get_conversations.json    GET  /webhook/wibot/conversations  │    │
│  │  • create_conversation.json  POST /webhook/wibot/conversation   │    │
│  │  • rename_conversation.json  PATCH                              │    │
│  │  • delete_conversation.json  DELETE                             │    │
│  │  • get_messages.json         GET  /webhook/wibot/messages       │    │
│  └─────────────────────────────────────────────────────────────────┘    │
│                                                                          │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │                    TOKENS & RAG                                  │    │
│  │  • get_user_tokens.json      GET  /webhook/wibot/user/tokens    │    │
│  │  • rag_ingestion.json        Ingestion documents RAG            │    │
│  └─────────────────────────────────────────────────────────────────┘    │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

### API REST - Endpoints

| Endpoint | Méthode | Authentification | Description |
|----------|---------|------------------|-------------|
| `/webhook/auth/login` | POST | Non | Connexion utilisateur |
| `/webhook/wibot/chat` | POST | JWT | Envoyer un message |
| `/webhook/wibot/conversations` | GET | JWT | Liste des conversations |
| `/webhook/wibot/messages` | GET | JWT | Messages d'une conversation |
| `/webhook/wibot/conversation/rename` | PATCH | JWT | Renommer conversation |
| `/webhook/wibot/conversation/delete` | DELETE | JWT | Supprimer conversation |
| `/webhook/wibot/user/tokens` | GET | JWT | Usage tokens |

### Sécurité

```
┌─────────────────────────────────────────────────────────────────────────┐
│                     MESURES DE SÉCURITÉ                                  │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  AUTHENTIFICATION                                                        │
│  ├── JWT (JSON Web Token)                                               │
│  │   ├── Algorithme: HS256                                              │
│  │   ├── Expiration: 8 heures                                           │
│  │   └── Secret: 32+ caractères                                         │
│  │                                                                       │
│  └── Mots de passe                                                      │
│      ├── Hashage: bcrypt                                                │
│      └── Rounds: 10                                                     │
│                                                                          │
│  PROTECTION API                                                          │
│  ├── Requêtes SQL paramétrées (anti-injection)                         │
│  ├── Rate limiting: 10 req/s par IP                                    │
│  ├── Headers CORS configurés                                            │
│  └── Headers de sécurité (X-Frame-Options, X-XSS-Protection)           │
│                                                                          │
│  RÉSEAU                                                                  │
│  ├── Réseau Docker isolé                                                │
│  └── Seul Nginx exposé (port 8080)                                     │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

---

## 6. Base de Données

### Schéma relationnel

```
┌─────────────────────────────────────────────────────────────────────────┐
│                      SCHÉMA POSTGRESQL                                   │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  ┌─────────────────────┐         ┌─────────────────────┐                │
│  │       users         │         │   user_token_usage  │                │
│  ├─────────────────────┤         ├─────────────────────┤                │
│  │ PK user_id          │────────►│ FK user_id          │                │
│  │    username         │         │    month            │                │
│  │    password_hash    │         │    used_tokens      │                │
│  │    email            │         │    quota_tokens     │                │
│  │    role             │         │ UNIQUE(user,month)  │                │
│  │    created_at       │         └─────────────────────┘                │
│  │    is_active        │                                                 │
│  └─────────┬───────────┘                                                 │
│            │                                                             │
│            │ 1:N                                                         │
│            ▼                                                             │
│  ┌─────────────────────┐                                                 │
│  │   conversations     │                                                 │
│  ├─────────────────────┤                                                 │
│  │ PK conversation_id  │ (UUID)                                         │
│  │ FK user_id          │                                                 │
│  │    title            │                                                 │
│  │    created_at       │                                                 │
│  │    updated_at       │ (auto-update trigger)                          │
│  └─────────┬───────────┘                                                 │
│            │                                                             │
│            │ 1:N                                                         │
│            ▼                                                             │
│  ┌─────────────────────┐                                                 │
│  │      messages       │                                                 │
│  ├─────────────────────┤                                                 │
│  │ PK message_id       │                                                 │
│  │ FK conversation_id  │                                                 │
│  │ FK user_id          │                                                 │
│  │    role             │ CHECK ('user' | 'assistant')                   │
│  │    content          │                                                 │
│  │    tokens           │                                                 │
│  │    attachments      │ (JSONB)                                        │
│  │    created_at       │                                                 │
│  └─────────────────────┘                                                 │
│                                                                          │
│  INDEX:                                                                  │
│  • idx_conversations_user (user_id)                                     │
│  • idx_messages_conversation (conversation_id)                          │
│  • idx_messages_user (user_id)                                          │
│  • idx_token_usage_user_month (user_id, month)                         │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

### Quota de tokens

- **Quota par défaut** : 50 000 tokens / mois / utilisateur
- **Suivi** : Table `user_token_usage` avec UPSERT mensuel
- **Vérification** : Avant chaque appel IA dans `chat_main`

---

## 7. Prérequis et Dépendances

### Logiciels requis

```
┌─────────────────────────────────────────────────────────────────────────┐
│                     PRÉREQUIS SYSTÈME                                    │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  OBLIGATOIRES                                                            │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │                                                                  │    │
│  │  🐳 Docker Desktop                                               │    │
│  │     Version: 24.0+ (avec Docker Compose v2)                     │    │
│  │     Rôle: Conteneurisation de tous les services backend         │    │
│  │     Download: https://www.docker.com/products/docker-desktop    │    │
│  │                                                                  │    │
│  │  📦 Node.js                                                      │    │
│  │     Version: 18.x LTS ou 20.x LTS                               │    │
│  │     Rôle: Exécution du serveur de développement frontend        │    │
│  │     Download: https://nodejs.org                                 │    │
│  │                                                                  │    │
│  │  📦 npm                                                          │    │
│  │     Version: 9.x+ (inclus avec Node.js)                         │    │
│  │     Rôle: Gestionnaire de paquets JavaScript                    │    │
│  │                                                                  │    │
│  │  🔑 Clé API Mistral                                              │    │
│  │     Rôle: Accès aux modèles IA (Mistral Large, Devstral)        │    │
│  │     Obtenir: https://console.mistral.ai                         │    │
│  │                                                                  │    │
│  └─────────────────────────────────────────────────────────────────┘    │
│                                                                          │
│  OPTIONNELS (pour développement)                                        │
│  ┌─────────────────────────────────────────────────────────────────┐    │
│  │                                                                  │    │
│  │  💻 VS Code                                                      │    │
│  │     Extensions recommandées:                                     │    │
│  │     - ESLint                                                     │    │
│  │     - Tailwind CSS IntelliSense                                 │    │
│  │     - TypeScript                                                 │    │
│  │     - Docker                                                     │    │
│  │                                                                  │    │
│  │  🔧 Git                                                          │    │
│  │     Pour le versioning du code                                   │    │
│  │                                                                  │    │
│  └─────────────────────────────────────────────────────────────────┘    │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

### Ressources système recommandées

| Ressource | Minimum | Recommandé |
|-----------|---------|------------|
| **RAM** | 4 Go | 8 Go |
| **CPU** | 2 cores | 4 cores |
| **Disque** | 10 Go | 20 Go |
| **OS** | Windows 10, macOS 10.15, Linux | - |

### Ports utilisés

| Port | Service | Accessible depuis |
|------|---------|-------------------|
| 8080 | Nginx (Production) | Navigateur |
| 5173 | Vite (Développement) | Navigateur |
| 5679 | n8n (Interface admin) | Navigateur |
| 5433 | PostgreSQL | Interne Docker |

---

## 8. Installation et Démarrage

### Installation rapide

```bash
# 1. Cloner le projet (si pas déjà fait)
git clone <repo-url>
cd WIBOT

# 2. Configurer les variables d'environnement
cd wibot-backend
cp env.example .env
# Éditer .env et ajouter votre MISTRAL_API_KEY

# 3. Démarrer les services backend
docker compose up -d

# 4. Attendre l'initialisation (~15 secondes)
sleep 15

# 5. Installer les dépendances frontend
cd ../wibot-frontend
npm install

# 6. Lancer le serveur de développement
npm run dev
```

### Accès aux services

```
┌─────────────────────────────────────────────────────────────────────────┐
│                         URLS D'ACCÈS                                     │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  🌐 Application WIBOT (Dev)                                             │
│     URL: http://localhost:5173                                          │
│     Identifiants test: khora / test123                                  │
│                                                                          │
│  🌐 Application WIBOT (Prod)                                            │
│     URL: http://localhost:8080                                          │
│                                                                          │
│  ⚙️ Interface n8n (Admin workflows)                                     │
│     URL: http://localhost:5679                                          │
│     Identifiants: admin / wibot_admin_2024                              │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

### Scripts disponibles

| Script | Commande | Description |
|--------|----------|-------------|
| Démarrage | `./start.sh` (Linux/Mac) ou `start.bat` (Windows) | Lance tout le stack |
| Arrêt | `./stop.sh` ou `stop.bat` | Arrête tous les services |
| Frontend dev | `npm run dev` | Serveur dev avec hot-reload |
| Frontend build | `npm run build` | Build production |

---

## 9. Expérience Utilisateur

### Parcours utilisateur

```
┌─────────────────────────────────────────────────────────────────────────┐
│                    PARCOURS UTILISATEUR WIBOT                            │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  1. CONNEXION                                                            │
│  ┌────────────────────────────────────────┐                             │
│  │           Page de Login                 │                             │
│  │  ┌────────────────────────────────┐    │                             │
│  │  │  Nom d'utilisateur: [______]   │    │                             │
│  │  │  Mot de passe:      [______]   │    │                             │
│  │  │         [Se connecter]         │    │                             │
│  │  └────────────────────────────────┘    │                             │
│  └────────────────────────────────────────┘                             │
│                         │                                                │
│                         ▼ Succès                                         │
│  2. INTERFACE CHAT                                                       │
│  ┌────────────────────────────────────────────────────────────────┐     │
│  │ ┌──────┐  WIBOT                    [🎫 45,000 tokens] [👤▼]    │     │
│  │ └──────┘                                                        │     │
│  │ ┌───────────┬──────────────────────────────────────────────────┐│     │
│  │ │ [+ Nouv.] │                                                  ││     │
│  │ │           │   Bienvenue! Comment puis-je vous aider ?        ││     │
│  │ │ 📝 Conv 1 │                                                  ││     │
│  │ │ 📝 Conv 2 │                                                  ││     │
│  │ │           │                                                  ││     │
│  │ └───────────┴──────────────────────────────────────────────────┘│     │
│  │ ┌──────────────────────────────────────────────────────────────┐│     │
│  │ │ [Mode ▼]  [Tapez votre message...              ] [Envoyer]   ││     │
│  │ └──────────────────────────────────────────────────────────────┘│     │
│  └────────────────────────────────────────────────────────────────┘     │
│                                                                          │
│  3. ACTIONS DISPONIBLES                                                  │
│  ┌────────────────────────────────────────────────────────────────┐     │
│  │  ✅ Nouvelle conversation                                       │     │
│  │  ✅ Envoyer message texte                                       │     │
│  │  ✅ Joindre fichiers (PDF, TXT, MD, CSV, JSON)                  │     │
│  │  ✅ Changer de mode (Code, Flash, Pro)                          │     │
│  │  ✅ Consulter historique conversations                          │     │
│  │  ✅ Renommer/Supprimer conversations                            │     │
│  │  ✅ Voir consommation tokens                                    │     │
│  │  ✅ Se déconnecter                                              │     │
│  └────────────────────────────────────────────────────────────────┘     │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

### Fonctionnalités clés

| Fonctionnalité | Description |
|----------------|-------------|
| **Chat IA** | Conversation avec l'IA Mistral, contexte WIDIP |
| **Historique** | Conservation des conversations précédentes |
| **Upload fichiers** | Drag & drop de fichiers → ingestion RAG temporaire |
| **Modes de chat** | Code, Flash, Pro selon le besoin |
| **Markdown** | Réponses formatées avec code coloré |
| **Copie code** | Bouton copier sur les blocs de code |
| **Quota tokens** | Suivi de consommation mensuel |
| **Responsive** | Interface adaptée desktop |

---

## 10. RAG - Système Déjà Implémenté

### Workflow RAG Ingestion v2 - Multi-Format

Le système RAG est **déjà opérationnel** avec un workflow complet :

```
┌─────────────────────────────────────────────────────────────────────────┐
│                    RAG INGESTION v2 - EXISTANT ✅                        │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  FORMATS SUPPORTÉS :                                                     │
│  ├── .md     Markdown                                                   │
│  ├── .txt    Texte brut                                                 │
│  ├── .pdf    PDF (extraction native n8n)                                │
│  ├── .docx   Word (extraction native n8n)                               │
│  ├── .doc    Word legacy                                                │
│  ├── .xlsx   Excel (conversion lignes → texte)                          │
│  ├── .xls    Excel legacy                                               │
│  ├── .csv    CSV (détection colonnes automatique)                       │
│  ├── .json   JSON (aplatissement récursif)                              │
│  └── .html   HTML (nettoyage balises automatique)                       │
│                                                                          │
│  CATÉGORISATION AUTOMATIQUE :                                            │
│  ┌─────────────────┬──────────────────────────────────────────────┐     │
│  │ Chemin/Préfixe  │ Catégorie assignée                           │     │
│  ├─────────────────┼──────────────────────────────────────────────┤     │
│  │ /procedures/    │ procedure                                    │     │
│  │ PROC_*          │ procedure                                    │     │
│  │ /clients/       │ client                                       │     │
│  │ CLIENT_*        │ client                                       │     │
│  │ /tickets/       │ ticket                                       │     │
│  │ TICKET_*        │ ticket                                       │     │
│  │ /documentation/ │ documentation                                │     │
│  │ DOC_*           │ documentation                                │     │
│  │ /faq/           │ faq                                          │     │
│  │ Autres          │ general                                      │     │
│  └─────────────────┴──────────────────────────────────────────────┘     │
│                                                                          │
│  MODES DE DÉCLENCHEMENT :                                                │
│  ├── Manuel     → Full refresh (clear + ré-ingestion complète)          │
│  ├── Webhook    → POST /webhook/wibot/rag/ingest (incrémental/full)    │
│  └── Cron       → Tous les dimanches à 3h (incrémental)                 │
│                                                                          │
│  PIPELINE TECHNIQUE :                                                    │
│  ┌─────────┐   ┌──────────┐   ┌─────────┐   ┌──────────┐   ┌─────────┐ │
│  │  Scan   │──►│ Switch   │──►│ Extract │──►│  Chunk   │──►│ Mistral │ │
│  │Directory│   │Extension │   │ Content │   │(overlap) │   │ Embed   │ │
│  └─────────┘   └──────────┘   └─────────┘   └──────────┘   └────┬────┘ │
│                                                                  │      │
│                                                                  ▼      │
│                                                           ┌──────────┐  │
│                                                           │ PGVector │  │
│                                                           │  Store   │  │
│                                                           └──────────┘  │
│                                                                          │
│  MÉTADONNÉES STOCKÉES :                                                  │
│  ├── source    (nom du fichier)                                         │
│  ├── category  (catégorie auto-détectée)                                │
│  ├── path      (chemin relatif)                                         │
│  └── hash      (MD5 pour détection changements)                         │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

### Endpoint RAG disponible

```bash
# Déclencher une ingestion manuelle
curl -X POST http://localhost:8080/webhook/wibot/rag/ingest \
  -H "Content-Type: application/json" \
  -d '{"mode": "full", "clear": true}'

# Ingestion incrémentale
curl -X POST http://localhost:8080/webhook/wibot/rag/ingest \
  -d '{"mode": "incremental"}'
```

### Pièces jointes - Système Implémenté

**État actuel : FONCTIONNEL ✅**

```
┌─────────────────────────────────────────────────────────────────────────┐
│                    PIÈCES JOINTES - IMPLÉMENTÉ                           │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  FRONTEND (InputBar.tsx)                                                 │
│  ├── ✅ Interface drag & drop fonctionnelle                             │
│  ├── ✅ Lecture fichiers en base64                                      │
│  ├── ✅ Envoi via API avec files: [{name, content}]                     │
│  └── ✅ Formats acceptés: PDF, TXT, MD, CSV, JSON, DOCX, XLSX           │
│                                                                          │
│  BACKEND (chat_main.json)                                                │
│  ├── ✅ Extraction de body.files dans Verify JWT                        │
│  ├── ✅ Node "Process Files" : decode base64 et sauvegarde              │
│  ├── ✅ Appel HTTP vers rag_ingestion avec category="temp"              │
│  └── ✅ Fichiers ingérés dans PGVector avec conversation_id             │
│                                                                          │
│  NETTOYAGE (delete_conversation.json)                                    │
│  ├── ✅ DELETE FROM n8n_vectors WHERE category='temp' AND conv_id       │
│  └── ✅ Suppression du dossier n8n-files/wibot-uploads/{conv_id}/       │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

**Architecture : RAG Temporaire par Conversation**

```
┌─────────────────────────────────────────────────────────────────────────┐
│              SOLUTION : INGESTION RAG TEMPORAIRE                         │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  1. UPLOAD (modification chat_main.json)                                 │
│  ┌───────────────────────────────────────────────────────────────┐      │
│  │  Fichier glissé → base64 → Backend                             │      │
│  │       │                                                        │      │
│  │       ▼                                                        │      │
│  │  Décoder base64 → Extraire texte (PDF/DOCX/TXT/etc)           │      │
│  │       │                                                        │      │
│  │       ▼                                                        │      │
│  │  Chunking + Embedding Mistral                                  │      │
│  │       │                                                        │      │
│  │       ▼                                                        │      │
│  │  INSERT INTO n8n_vectors                                       │      │
│  │    metadata = {                                                │      │
│  │      "category": "temp",                                       │      │
│  │      "conversation_id": "uuid-conv",                           │      │
│  │      "source": "document.pdf",                                 │      │
│  │      "user_id": 123                                            │      │
│  │    }                                                           │      │
│  │       │                                                        │      │
│  │       ▼                                                        │      │
│  │  IA a maintenant accès au contenu via RAG                     │      │
│  └───────────────────────────────────────────────────────────────┘      │
│                                                                          │
│  2. NETTOYAGE (modification delete_conversation.json)                    │
│  ┌───────────────────────────────────────────────────────────────┐      │
│  │  DELETE FROM n8n_vectors                                       │      │
│  │  WHERE metadata->>'category' = 'temp'                          │      │
│  │    AND metadata->>'conversation_id' = :conv_id                 │      │
│  └───────────────────────────────────────────────────────────────┘      │
│                                                                          │
│  3. RECHERCHE (le RAG filtre automatiquement)                            │
│  ┌───────────────────────────────────────────────────────────────┐      │
│  │  Documents permanents : toujours accessibles                   │      │
│  │  Documents temp : uniquement pour leur conversation            │      │
│  └───────────────────────────────────────────────────────────────┘      │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

---

## 11. Pistes d'Amélioration

### Ce qui reste à faire

```
┌─────────────────────────────────────────────────────────────────────────┐
│                    ROADMAP D'AMÉLIORATION                                │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  🔐 AUTHENTIFICATION & AUTORISATION                                     │
│  ├── Intégration SSO (Active Directory / LDAP)                         │
│  ├── Matrice d'habilitation (rôles et permissions)                     │
│  ├── MFA (Multi-Factor Authentication)                                  │
│  └── Session management avancé                                          │
│                                                                          │
│  ✅ PIÈCES JOINTES - IMPLÉMENTÉ                                         │
│  ├── ✅ Traitement des fichiers uploadés                                │
│  ├── ✅ Ingestion RAG temporaire par conversation                       │
│  ├── ✅ Nettoyage automatique à la suppression de conversation          │
│  └── ✅ Support PDF, DOCX, TXT, MD, CSV, JSON, XLSX                     │
│                                                                          │
│  📚 RAG - AMÉLIORATIONS (base déjà fonctionnelle)                       │
│  ├── Filtrage par permissions utilisateur                               │
│  ├── Connecteurs sources externes (GLPI, SharePoint, Confluence)        │
│  ├── Interface d'upload admin pour nouveaux documents                   │
│  └── Statistiques d'utilisation du RAG                                  │
│                                                                          │
│  🎛️ ADMINISTRATION                                                       │
│  ├── Interface d'administration                                         │
│  ├── Gestion des utilisateurs                                           │
│  ├── Configuration des quotas                                           │
│  ├── Monitoring et logs                                                 │
│  └── Statistiques d'utilisation                                         │
│                                                                          │
│  🔧 TECHNIQUE                                                            │
│  ├── Tests automatisés (Jest, Playwright)                               │
│  ├── CI/CD pipeline                                                     │
│  ├── Monitoring (Prometheus, Grafana)                                   │
│  ├── Backup automatisé                                                  │
│  └── HTTPS en production                                                 │
│                                                                          │
│  📱 UX/UI                                                                │
│  ├── Mode responsive mobile                                             │
│  ├── Mode clair/sombre                                                  │
│  ├── Raccourcis clavier                                                 │
│  ├── Export conversations (PDF, Markdown)                               │
│  └── Suggestions de prompts                                             │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

### Matrice d'habilitation proposée

```
┌─────────────────────────────────────────────────────────────────────────┐
│                    MATRICE D'HABILITATION                                │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  RÔLES PROPOSÉS:                                                        │
│  ┌────────────────┬─────────────────────────────────────────────────┐   │
│  │ Rôle           │ Description                                      │   │
│  ├────────────────┼─────────────────────────────────────────────────┤   │
│  │ admin          │ Accès total, gestion utilisateurs, config       │   │
│  │ tech_senior    │ Accès RAG complet, tous les modes IA            │   │
│  │ tech           │ Accès RAG documentation + procédures            │   │
│  │ support        │ Accès RAG procédures + tickets                  │   │
│  │ guest          │ Chat simple, pas de RAG                         │   │
│  └────────────────┴─────────────────────────────────────────────────┘   │
│                                                                          │
│  PERMISSIONS:                                                            │
│  ┌────────────────┬───────┬────────┬──────┬─────────┬───────┐          │
│  │ Permission     │ admin │ senior │ tech │ support │ guest │          │
│  ├────────────────┼───────┼────────┼──────┼─────────┼───────┤          │
│  │ Chat IA        │   ✅  │   ✅   │  ✅  │   ✅    │  ✅   │          │
│  │ RAG Docs       │   ✅  │   ✅   │  ✅  │   ❌    │  ❌   │          │
│  │ RAG Procédures │   ✅  │   ✅   │  ✅  │   ✅    │  ❌   │          │
│  │ RAG Clients    │   ✅  │   ✅   │  ❌  │   ❌    │  ❌   │          │
│  │ RAG Tickets    │   ✅  │   ✅   │  ❌  │   ✅    │  ❌   │          │
│  │ Mode Pro       │   ✅  │   ✅   │  ✅  │   ❌    │  ❌   │          │
│  │ Upload files   │   ✅  │   ✅   │  ✅  │   ✅    │  ❌   │          │
│  │ Admin panel    │   ✅  │   ❌   │  ❌  │   ❌    │  ❌   │          │
│  │ Gestion users  │   ✅  │   ❌   │  ❌  │   ❌    │  ❌   │          │
│  │ Config quotas  │   ✅  │   ❌   │  ❌  │   ❌    │  ❌   │          │
│  └────────────────┴───────┴────────┴──────┴─────────┴───────┘          │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

---

## Annexes

### Variables d'environnement

```bash
# Base de données
POSTGRES_DB=wibot
POSTGRES_USER=widip
POSTGRES_PASSWORD=<mot_de_passe_sécurisé>

# n8n
N8N_HOST=localhost
N8N_PORT=5678
N8N_BASIC_AUTH_USER=admin
N8N_BASIC_AUTH_PASSWORD=<mot_de_passe_admin>

# Sécurité
JWT_SECRET=<clé_minimum_32_caractères>

# IA
MISTRAL_API_KEY=<votre_clé_api_mistral>
```

### Commandes utiles

```bash
# Voir les logs des containers
docker compose logs -f

# Redémarrer un service spécifique
docker compose restart n8n

# Accéder à PostgreSQL
docker exec -it wibot-postgres psql -U widip -d wibot

# Rebuild frontend
cd wibot-frontend && npm run build

# Copier build vers backend
cp -r dist/* ../wibot-backend/frontend/
```

---

**Document créé le 31/12/2024 - WIBOT v1.0**
